var applican = null;
var applican_debug_settings = require('applican_debug_settings');
...
(function() {
	...
	this.pf_callApi = function(scheme, callbackStack, success, fail){
		...
		if(!this.is_wait){
		    this.is_wait = true;
		    setTimeout('applican.queue.pf_callSchemeQueue()', this.PF_QUEUE_INTERVAL);
		}
		return _callbackId;
	    }
	...
	this._getCurrentPosition = function() {
	    /*alert('xxx');
	     alert(this.geolocationOptions.APIKey1);
	     alert(this.geolocationOptions.APIKey2);
	     */
	...
	this._receiveResponse = function(xhr) {
		...
		err.code = code;
		message = this._getFirstChildText(xml, 'Message');
		err.message = message;
	...
	this._init = function(){
	    if(this.config.debug){
		var device_info = {name:"", platform:"", uuid:"", version:"", applican:""};
...
function applican_init(){
    setTimeout('applican._init()',1500);
}
